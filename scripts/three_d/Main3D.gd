...
@export var screen_size: Vector2 = Vector2(16.0, 9.0)
@export var screen_tilt_degrees: float = -5.0
...
var _flat_texture_rect: TextureRect = null
...
func _build_table() -> void:
	var floor := _make_box(
		"StoneFloor",
		Vector3(24.0, 0.16, 17.0),
		Vector3(0.0, -3.05, -1.2),
	...
	var table := _make_box(
		"GameTable",
		Vector3(18.4, 0.34, 6.4),
		Vector3(0.0, -2.68, 0.45),
	...
	var rear_rail := _make_box(
		"RearTableRail",
		Vector3(18.8, 0.44, 0.32),
		Vector3(0.0, -2.34, -2.58),
...
func _build_game_screen() -> void:
	...
	rig.position = Vector3(0.0, 1.9, 0.0)
...
func _build_camera() -> void:
	...
	_camera.position = Vector3(0.0, 2.15, 13.5)
	_camera.fov = 47.0
...
func _build_flat_canvas() -> void:
	...
	_flat_texture_rect = TextureRect.new()
	_flat_texture_rect.name = "FlatGameView"
	_flat_texture_rect.texture = _game_viewport.get_texture()
	_flat_texture_rect.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	_flat_texture_rect.stretch_mode = TextureRect.STRETCH_SCALE
	_flat_texture_rect.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_flat_canvas_layer.add_child(_flat_texture_rect)
...
func _refresh_display_mode() -> void:
	...
	if wants_flat_2d == _is_flat_2d_mode:
		return
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion:
		if _is_flat_2d_mode:
			_forward_flat_mouse_motion(event as InputEventMouseMotion)
		else:
			_forward_mouse_motion(event as InputEventMouseMotion)
		return
	if event is InputEventMouseButton:
		if _is_flat_2d_mode:
			_forward_flat_mouse_button(event as InputEventMouseButton)
		else:
...
func _forward_flat_mouse_motion(event: InputEventMouseMotion) -> void:
	var game_position := _map_window_position_to_flat_game_viewport(event.position)
...
func _forward_flat_mouse_button(event: InputEventMouseButton) -> void:
	var game_position := _map_window_position_to_flat_game_viewport(event.position)
...
func _map_window_position_to_flat_game_viewport(window_position: Vector2) -> Vector2:
	var viewport_size := get_viewport().get_visible_rect().size
	if viewport_size.x <= 0.0 or viewport_size.y <= 0.0:
		return Vector2.ZERO
	var normalized := Vector2(
		clampf(window_position.x / viewport_size.x, 0.0, 1.0),
		clampf(window_position.y / viewport_size.y, 0.0, 1.0)
	)
	return Vector2(
		normalized.x * float(_game_viewport.size.x),
		normalized.y * float(_game_viewport.size.y)
	)
