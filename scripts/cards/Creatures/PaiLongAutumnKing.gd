...
func on_impact(game_manager: GameManager) -> void:
	...
	if prompt_host != null and prompt_host.has_method("_queue_pai_long_autumn_king_impact_prompt"):
		prompt_host.call("_queue_pai_long_autumn_king_impact_prompt", self)
		return

	var fallback_text := resolve_stormcloud_impact(game_manager, valid_targets[0])
	if game_manager != null:
