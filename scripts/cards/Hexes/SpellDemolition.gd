...
func on_activate_action(game_manager: GameManager, action: CardAction) -> void:
	...
	var target_name := target_spell.card_name
	if target_spell is Card:
		target_name = target_spell.get_target_log_display_name(game_manager.get_feedback_viewer())
	...
	if target_spell.current_zone != null and target_spell.current_zone != target_spell.card_owner.graveyard_zone:
		game_manager.request_send_to_graveyard(target_spell, Callable(), false, true)

	game_manager.note_player_feedback("%s silenced and destroyed %s." % [card_name, target_name])
	card_owner.move_card(self, card_owner.graveyard_zone)
