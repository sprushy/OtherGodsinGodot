...
func _can_reduce_card(
	...
	if target_card.card_type == Card.CardType.SPELL:
		return _has_friendly_mage_on_field()
...
func get_hover_detail_lines(viewer: Player = null) -> Array[String]:
	...
	if viewer != null and viewer != card_owner:
		return details
	details.append("Discount active." if _has_friendly_mage_on_field() else "Needs a friendly Mage on the field.")
	return details

func _has_friendly_mage_on_field() -> bool:
	if card_owner == null:
		return false
