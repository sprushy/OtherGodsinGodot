...
func _init() -> void:
	...
	targets = true
	ability_text = "[b]Unlock[/b] (5): Choose a friendly [b]Shapeshifter[/b]. It loses the ability to shapeshift. Summon a [b]Spirit[/b] token with the stats and classes its alternate form would have. When that Spirit leaves the field, [b]Relock[/b] this card."
	artist = "Trent Nguyen"
	art_path = ART_PATH
...
func on_unlock(game_manager: GameManager) -> void:
	...
	if prompt_host != null and prompt_host.has_method("_queue_tonal_extraction_prompt"):
		prompt_host.call("_queue_tonal_extraction_prompt", self)
		return
	activate(game_manager, valid_targets[0])
