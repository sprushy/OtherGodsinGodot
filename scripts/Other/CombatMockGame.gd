...
func _make_power_icon(card: Card, is_enemy: bool, player: Player) -> Control:
	...
	var can_show_hover := not (is_enemy and card.is_face_down and not (power != null and power.is_publicly_revealed))
	if can_show_hover:
	...
	if is_enemy:
		...
		var revealed_face_down := enemy_power != null and enemy_power.is_face_down and enemy_power.is_publicly_revealed
	...
	if can_unlock_now or activatable:
		panel.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
...
func _get_card_name_safe(card: Card, fallback: String = "Card") -> String:
	if card == null:
		return fallback
	return card.card_name

func _queue_magical_action(action_type: int, source_card: Card, target, resolution_text: String, resolve_callback: Callable) -> void:
...
func _queue_priority_event(event_name: String, source_card: Card = null, event_speed: int = 0, resolve_callback: Callable = Callable()) -> void:
	var action := CardAction.new()
...
func _queue_charm_action(charm: CharmCard, triggering_action: CardAction = null) -> void:
	...
	if from_hand:
		if not charm.can_activate_from_hand(game_manager, source_action):
			...
			return
		if not charm.pay_costs(game_manager.current_player):
...
func _on_hand_card_pressed(card: Card) -> void:
	...
	if card is CharmCard:
		if (card as CharmCard).must_be_prepared_to_activate:
			...
			action_label.text = "Selected charm: " + card.card_name + " - click a zone to play it, or right-click to prepare it."
		...
		if selected_card.card_type == Card.CardType.SPELL:
			if game_manager.can_play_card(game_manager.current_player, selected_card, zone):
				if selected_card is ApollyonsDemiurge or selected_card.card_name == "Apollyon's Demiurge":
					var _demiurge_spell = selected_card
					_queue_magical_action(
						CardAction.Type.SPELL,
						_demiurge_spell,
						null,
						"Apollyon's Demiurge goes on the stack.",
						func() -> void:
							_show_demiurge_prompt(_demiurge_spell)
					)
					...
					if spell.pay_costs(game_manager.current_player):
						_queue_magical_action(
							...
							func() -> void:
								...
								if spell.current_zone == game_manager.current_player.hand_zone:
...
func _do_place_creature(card: Card, zone: Zone, mode: String) -> void:
	if mode == "stealth":
		...
		if card is BlessedKnights:
			_show_blessed_knights_prompt(card as BlessedKnights)
...
func _on_board_card_pressed(card: Card) -> void:
	...
	if awaiting_god_ability_target and god_ability_source != null:
		if god_ability_source.has_method("is_valid_activation_target") and god_ability_source.is_valid_activation_target(card):
			...
			update_ui()
			action_label.text = card.card_name + " is immune to " + source_god.card_name + "." if was_immune else "God ability resolved."
		else:
	...
	if _pending_equip_action != "":
		if card.get_controller() == game_manager.other_player and can_intercept(card, _pending_equip_actor):
...
func _on_enemy_card_pressed(target_card: Card) -> void:
	...
	if awaiting_god_ability_target and god_ability_source != null:
		if god_ability_source.has_method("is_valid_activation_target") and god_ability_source.is_valid_activation_target(target_card):
			...
			awaiting_god_ability_target = false
			god_ability_source = null
			action_label.text = target_card.card_name + " is immune to " + source_god.card_name + "." if was_immune else "God ability resolved."
			update_ui()
		else:
			action_label.text = "Invalid target for " + god_ability_source.card_name + "."
		return
	...
	if _pending_equip_action != "":
		if target_card.get_controller() == game_manager.other_player and can_intercept(target_card, _pending_equip_actor):
			_remove_no_intercept_button()
	...
	if selected_attacker:
		if target_card.is_god:
			...
			action_label.text = _get_card_name_safe(selected_attacker, "A creature") + " attacks " + target_card.card_owner.player_name + "'s followers!"
			resolve_pending_attack()
			...
			action_label.text = _get_card_name_safe(selected_attacker, "A creature") + " attacking " + _get_card_name_safe(target_card, "an enemy card") + " - resolving..."
			resolve_pending_attack()
		else:
...
func _advance_attack_queue() -> void:
	while not _queued_attackers.is_empty():
		...
		action_label.text = attacker.card_name + " attacking followers Ã¢â‚¬â€ opponent may intercept"
...
func _on_creature_right_clicked(card: Card) -> void:
	...
	if can_activate_creature:
		...
		btn.pressed.connect(func():
			...
			action_label.text = card.card_name + " used " + (card.get_activation_label() if card.has_method("get_activation_label") else "its ability") + "."
			update_ui()
		)
		vbox.add_child(btn)
	...
	if can_stance:
		if card.is_stealth:
			var reveal_attack_btn := Button.new()
			reveal_attack_btn.text = "Reveal in Attack"
			reveal_attack_btn.pressed.connect(func():
				_close_context_menu()
				var was_stealth: bool = card.is_stealth
				if game_manager.creature_change_mode(card, Card.CreatureMode.ATTACK):
					action_label.text = card.card_name + " revealed in attack mode."
					update_ui()
					_handle_post_reveal_prompt(card, was_stealth)
				else:
					action_label.text = card.card_name + " could not reveal in attack mode."
			)
			vbox.add_child(reveal_attack_btn)

			var reveal_defense_btn := Button.new()
			reveal_defense_btn.text = "Reveal in Defense"
			reveal_defense_btn.pressed.connect(func():
				_close_context_menu()
				var was_stealth: bool = card.is_stealth
				if game_manager.creature_change_mode(card, Card.CreatureMode.DEFENSE):
					action_label.text = card.card_name + " revealed in defense mode."
					update_ui()
					_handle_post_reveal_prompt(card, was_stealth)
				else:
					action_label.text = card.card_name + " could not reveal in defense mode."
			)
			vbox.add_child(reveal_defense_btn)
		else:
...
func _bdrag_finish(drop_pos: Vector2) -> void:
	...
	if target_zu._is_enemy and target_zone.cards.size() > 0:
		...
		if target_card.card_type == Card.CardType.CREATURE or target_card.card_type == Card.CardType.STRUCTURE:
			selected_attacker = card
			pending_attack_target = target_card
			action_label.text = card.card_name + " attacking " + _get_card_name_safe(target_card, "an enemy card") + "..."
			resolve_pending_attack()
			return
...
func can_intercept(defender: Card, attacker: Card) -> bool:
	...
	if defender.creature_mode == Card.CreatureMode.DEFENSE:
		return true
	# Attack-mode creatures can intercept once per turn at equal or greater speed
	if defender.creature_mode == Card.CreatureMode.ATTACK and defender.can_take_major_creature_action():
...
func resolve_pending_attack() -> void:
	...
	action_label.text = _get_card_name_safe(action.attacker, "A creature") + " declares an attack on " + declared_target_name + "!"
	_offer_priority()
...
func check_for_possible_intercepts_for_equip_action() -> void:
	...
	for zone in defender.frontline_zones + defender.reserve_zones:
		for card in zone.cards:
			if can_intercept(card, _pending_equip_actor):
...
func resolve_pending_equip_action(interceptor: Card) -> void:
	...
	if interceptor != null:
		...
		game_manager.resolve_combat(actor, interceptor)
		update_ui()
		return
	if action == "steal":
...
func _show_priority_prompt(player: Player, responses: Array) -> void:
	...
	for response in responses:
		var btn := Button.new()
		btn.text = "Play " + _get_stack_card_type_label(response) + ": " + response.card_name
...
func _on_priority_response_chosen(card: Card) -> void:
	...
	if card is HexCard:
		...
		_queue_charm_action(card as CharmCard, game_manager.action_stack.back())
...
func _resolve_blessed_knights_impact(ward_kind: String) -> void:
	...
	action_label.text = card.card_name + " grants Blessed Ward against " + card.get_blessed_ward_label(ward_kind) + " this turn."
	update_ui()
...
func _resolve_book_of_life(chosen: Card) -> void:
	...
	if not spell.pay_costs(game_manager.current_player):
		...
		return
	game_manager.notify_spell_played(game_manager.current_player, spell)
	var summoned: Card = chosen
	spell.resolve(game_manager, chosen)
	if spell.current_zone == game_manager.current_player.hand_zone:
		game_manager.current_player.hand_zone.cards.erase(spell)
	game_manager.current_player.move_card(spell, game_manager.current_player.graveyard_zone)
	selected_card = null
	if summoned != null and summoned.current_zone != null and summoned.current_zone.zone_owner == game_manager.current_player:
		action_label.text = "Book of Life gained 10 followers and summoned " + summoned.card_name + " in silence."
	else:
		action_label.text = "Book of Life gained 10 followers."
	update_ui()
...
func _initiate_blot_with_sacrifice(spell, sacrifice_target: Card) -> void:
	...
	if spell.get_valid_hand_creatures(BlotSacrifice.MAX_SUMMON_LEVELS, []).is_empty():
		action_label.text = "Blot Sacrifice: no valid creatures in hand to summon."
		update_ui()
		return
...
func _show_blot_sacrifice_prompt(spell) -> void:
	...
	_show_card_selection_overlay(
		...
		func(chosen: Card) -> void:
			_pending_blot_spell = spell
			_pending_blot_sacrifice_target = chosen
			_pending_blot_selected_creatures.clear()
			_show_blot_creature_prompt()
	)
...
func _show_blot_creature_prompt() -> void:
	...
	var panel := PanelContainer.new()
	var style := StyleBoxFlat.new()
	style.bg_color = Color(0.10, 0.08, 0.08, 0.97)
	style.border_color = Color(0.78, 0.70, 0.60)
	for side in [SIDE_LEFT, SIDE_RIGHT, SIDE_TOP, SIDE_BOTTOM]:
		style.set_border_width(side, 2)
	panel.add_theme_stylebox_override("panel", style)
	panel.custom_minimum_size = Vector2(460, 0)
	...
	var info := Label.new()
	info.text = "Sacrificing: %s\nRemaining levels: %d / %d\nOpen zones: %d" % [
	...
	if not _pending_blot_selected_creatures.is_empty():
		...
		for chosen in _pending_blot_selected_creatures:
			var remove_btn := Button.new()
			remove_btn.text = "Remove " + chosen.card_name
			remove_btn.pressed.connect(func() -> void:
				_pending_blot_selected_creatures.erase(chosen)
				_show_blot_creature_prompt()
			)
	...
	var add_buttons_generated: int = 0
	for creature in valid_choices:
		if _pending_blot_selected_creatures.size() >= available_slots:
			break
		var add_btn := Button.new()
		add_btn.text = "Add %s (Lv %d)" % [creature.card_name, creature.level]
		add_btn.pressed.connect(func() -> void:
			_pending_blot_selected_creatures.append(creature)
			if _blot_has_more_summonable_creatures(spell):
				_show_blot_creature_prompt()
			else:
				_on_blot_sacrifice_confirm_pressed()
		...
		add_buttons_generated += 1
	# No add buttons were generated despite passing early-exit checks — auto-handle.
	if add_buttons_generated == 0:
		panel.free()
		if not _pending_blot_selected_creatures.is_empty():
			_on_blot_sacrifice_confirm_pressed()
		else:
	...
	var confirm_btn := Button.new()
	confirm_btn.text = "Summon Chosen"
...
func _on_blot_sacrifice_confirm_pressed() -> void:
	# Capture and clear blot state FIRST to prevent re-entry: pay_costs triggers mana_changed
	# which calls update_ui() -> _sync_blot_sacrifice_prompt_state() -> here again.
	var spell = _pending_blot_spell
	...
	if chosen_creatures.is_empty():
		action_label.text = "Blot Sacrifice cancelled: no creatures chosen to summon."
		update_ui()
		return
	if spell.get_available_summon_zones().is_empty():
		action_label.text = "Blot Sacrifice: no open zone to summon into."
		update_ui()
		return
	# Validate sacrifice target is still on the field
	if sacrifice_target.current_zone == null or not sacrifice_target.current_zone.is_board_zone():
		action_label.text = "Blot Sacrifice cancelled: sacrifice target is no longer on the field."
		update_ui()
		return
	# Validate full cost (mana + creature sacrifice) before consuming anything
	if not game_manager.can_play_card(game_manager.current_player, spell, null):
	...
	var paid: bool = spell.pay_costs(game_manager.current_player)
	spell.creature_sacrifice_cost = orig_creature_cost
	if not paid:
		action_label.text = "Cannot cast Blot Sacrifice."
		update_ui()
		return
	game_manager._send_to_graveyard_with_hook(sacrifice_target)
	_queue_magical_action(
		CardAction.Type.SPELL,
		spell,
		null,
		...
		func() -> void:
			game_manager.notify_spell_played(game_manager.current_player, spell)
			spell.summon_selected_creatures(game_manager, chosen_creatures)
			if spell.current_zone == game_manager.current_player.hand_zone:
				game_manager.current_player.hand_zone.cards.erase(spell)
			game_manager.current_player.move_card(spell, game_manager.current_player.graveyard_zone)
	)
...
func _on_demiurge_confirm_pressed(spin: SpinBox) -> void:
	...
	var demon_choices: Array = spell.resolve_with_x(game_manager, x_value)
...
func _on_retreat_no() -> void:
	...
	if blocked_ask != null:
		...
		action_label.text = _get_card_name_safe(action.attacker) + " fought " + _get_card_name_safe(defender) + "!"
	...
	match action.type:
		CardAction.Type.ABILITY:
			if action.card is HexCard:
				...
				action_label.text = action.card.card_name + " resolved!"

		CardAction.Type.SPELL:
			if action.resolve_callback.is_valid():
				action.resolve_callback.call()
				action_label.text = action.card.card_name + " [" + _get_stack_card_type_label(action.card) + "] resolved!"
		...
		CardAction.Type.ATTACK:
			...
			if actual_target is Card:
				...
				if blocked_ask != null:
					...
					action_label.text = _get_card_name_safe(action.attacker) + " fought " + _get_card_name_safe(actual_target) + "!"
				...
				action_label.text = _get_card_name_safe(action.attacker) + " dealt " + str(action.attacker.get_effective_strength()) + " damage to followers!"
			action.attacker.spend_major_creature_action()
