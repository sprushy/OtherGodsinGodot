...
static func ensure_registered() -> bool:
	var target_height := UIArtScaler.get_board_cursor_target_height(CURSOR_TARGET_HEIGHT)
	if _registered and _registered_target_height == target_height:
		return true
	if CURSOR_SOURCE == null:
		return false
	var cursor_texture := UIArtScaler.build_cursor_texture(CURSOR_SOURCE, target_height)
	if cursor_texture == null:
		return false
	var hotspot := UIArtScaler.get_cursor_hotspot(cursor_texture, CURSOR_HOTSPOT_RATIO)
